...
const isUserRegistered = async (keyPair) => {
    ...
    const hash = await contract.getMessageHash(address, ethers.utils.toUtf8Bytes("1000"), message, 1);
    const signature = await ACCOUNT_WALLET.signMessage(ethers.utils.arrayify(hash));
    console.log("------------------- aaa ----------------");
    console.log(address, publickey, signature,"1000");
    const data = await contractWithSigner.test1(address, publickey, signature, ethers.utils.toUtf8Bytes("1000"));
