...
export class DigitalArtsService {
  ...
  async deployNewNftContract() {
    ...
    const tx = await digitalArtFactory.deploy( Config.USER_REG_CONTRACT_ADDRESS, (Config.USER_REG_ENABLE).toLowerCase() === 'true');
    this.refreshContractAddress(tx.address);
    console.log("Newly deployed nft ---> : ", tx.address);
  }
  ...
  async mint(title: string, artist: string, imageUrl: string) {
    ...
    let userRegContractAddr = Config.USER_REG_CONTRACT_ADDRESS;
    let userRegEnable = Config.USER_REG_ENABLE;
    console.log("contract address  ---> : ", userRegContractAddr);
    console.log("userEnable  ---> : ", userRegEnable);
    let transaction;
    try {
