...
interface PrivacyVirtualContract {
...
contract PrivateToken {
    ...
    mapping(uint64 => uint64) _sigIdToTxId;

    PrivacyVirtualContract _vc;
    ...
    constructor(bytes memory config) {
        bytes memory payload = abi.encode("PrivacyVirtualContract");
        ...
        bytes memory baddr = abi.decode(result, (bytes));
        address addr;
        assembly {
            addr := mload(add(baddr, 20))
        }
        _vc = PrivacyVirtualContract(addr);
    ...
    function checkPublicBudget(string memory userId) external view returns (bool) {
        return _budgets[userId].isSet;
    }
    ...
    function convertPublicToPrivate(PublicToPrivateRequest calldata req) public returns (bool) {
        ...
        _sigIdToTxId[nextSigId] = nextTxNum;
    ...
    function getTransactionSigs(uint64 txNum) external view returns (bytes[] memory) {
        ...
        for (uint i = 0; i < _ledger[txNum].sigIds.length; ++i) {
            ...
            if (result[i].length == 0) return new bytes[](0);
        }

        return result;
    }

    function getTransactionSignaturesCount(uint64 txNum) external view returns (uint) {
        require(_ledger[txNum].txType != TxType.NotSet);

        return _ledger[txNum].sigIds.length;
    }
