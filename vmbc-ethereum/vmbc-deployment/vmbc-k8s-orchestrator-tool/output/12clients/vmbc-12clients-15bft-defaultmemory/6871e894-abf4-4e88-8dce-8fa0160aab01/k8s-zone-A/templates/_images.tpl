...
{{- define "cre.image" -}}
{{ include "common.images.image" (dict "imageRoot" .Values.cre.image "global" .Values.global) }}
{{- end -}}
...
{{ include "common.images.image" (dict "imageRoot" .Values.telegraf.image "global" .Values.global) }}
{{- end -}}

{{- define "telegraf.urlConcatenation" -}}
{{- $list := list }}
{{- $parts := split "," .Values.telegraf.endpoints }}
{{- range  $parts }}
{{- $list = append $list (printf "\"%s\"" .) }}
{{- end }}
{{- $opList := join ", " $list }}
{{- $opList := printf "%s %s %s" "[" $opList  "]" }}
{{- $opList }}
{{- end }}
